...
# remove the results csv file from the list
directory_files = directory_files[-length(directory_files)]
print(directory_files)
num_tiffs = length(directory_files)
# hold the results
all_results_df = data.frame(matrix(data = 0, nrow = num_tiffs, ncol = 12))
colnames(all_results_df) = c("sample", "method","threshold","snr","cnr","fg_mean","fg_std","bg_mean","bg_std","foreground_fraction","score","status")
...
for(i in 1:num_tiffs){
  ...
  current_results_df = filter(current_results_df,current_results_df$method == "isodata")
  all_results_df[i, 1] = directory_files[i]
  all_results_df[i,2:12] = current_results_df[1, ]
}

# save results
save_directory = paste0(getwd(),"/","all_isodata_thresholding_results.csv")
